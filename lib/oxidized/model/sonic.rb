class SONiC < Oxidized::Model
  ...
  cmd 'sudo show runningconfiguration all' do |cfg|
    cfg += add_comment 'SONiC Configuration Database'
    cfg += cfg.each_line.to_a[3..-1].join
    cfg
  end

  cmd 'vtysh -c "show running-config"' do |cfg|
    cfg += add_comment 'SONiC frr running-config'
    cfg += cfg.each_line.to_a[3..-1].join
    cfg
  end
